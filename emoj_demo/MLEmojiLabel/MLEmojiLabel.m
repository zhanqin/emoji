...
+ (NSDictionary *)emojiDictionary {
    ...
    static dispatch_once_t onceToken;
	dispatch_once(&onceToken, ^{
	    NSString *emojiFilePath = [[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"emtions.plist"];
...
//                                                           range:NSMakeRange(0, [emojiText length])];
    ...
    NSMutableArray * emojisTemp = [NSMutableArray arrayWithCapacity:0];
    int startLoctaion = 0;
    int endLocation = 0;
    for (int i = 0; i < emojiText.length; i ++) {
        if ([[emojiText substringWithRange:NSMakeRange(i, 1)] isEqualToString:@"["]) {
            //起始位置
            startLoctaion = i;
        }
        if ([[emojiText substringWithRange:NSMakeRange(i, 1)] isEqualToString:@"]"]) {
        ...
        if (endLocation) {
            NSString * expression = [emojiText substringWithRange:NSMakeRange(startLoctaion, endLocation - startLoctaion + 1)];
            if ([EmojiHelper isEmojiString:expression]) {
                BZMLEmojiModel * model = [[BZMLEmojiModel alloc] init];
                model.location = startLoctaion;
                model.length = endLocation - startLoctaion + 1;
                [emojisTemp addObject:model];
                startLoctaion = 0;
                endLocation = 0;
            }else{
                startLoctaion = 0;
    ...
    emojis = [emojisTemp copy];
    NSMutableAttributedString *attrStr = [[NSMutableAttributedString alloc] init];
    NSUInteger location = 0;
    
    
    CGFloat emojiWith = self.font.lineHeight*kEmojiWidthRatioWithLineHeight;
    for (BZMLEmojiModel * model in emojis) {
        NSRange range = NSMakeRange(model.location, model.length);
